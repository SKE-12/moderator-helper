...
import { Allegiance, GamePhase, RoleName } from "../models/Allegiance";
...
export default class GameState {
  ...
  public getAlivePlayer(): Player[] {
    return this.players.filter(
      (player) => player.isAlive && !_.isEmpty(player.playerName)
    );
  }
  ...
  public endNight() {
    ...
    for (const deadPlayer of killed) {
      ...
      if (isDead) {
        this.nightSummary.annoucement.push(
          "Player: " + deadPlayer.playerName + "ถูกฆ่าตายในคืนนี้"
        );
      }
    }
    if (this.huntressTarget) {
      this.nightSummary.annoucement.push(
        "Player: " + this.huntressTarget.playerName + "ถูกฆ่าตายในคืนนี้"
      );
    }
  }
  ...
  private checkWinCondition(): Allegiance | null {
    ...
    for (const player of alivePlayer) {
      ...
      if (player.role === RoleName.PLAGUE_DOCTOR) {
        plagueDoctor++;
      }
    }
    if (plagueDoctor === 0) {
